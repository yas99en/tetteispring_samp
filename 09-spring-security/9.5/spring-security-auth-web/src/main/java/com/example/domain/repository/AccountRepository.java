...
public class AccountRepository {
	...
	// 9.5.5 メソッドへの認可
	public Account findOneByName(String username) {
		String sql = "SELECT login_user.name as name, login_user.password as password, roles.id as role_id, roles.name as role_name FROM login_user, user_role, roles WHERE login_user.name = ? AND login_user.id = user_role.user_id AND user_role.role_id = roles.id";
		List<Map<String, Object>> resultList = jdbcTemplate.queryForList(sql, username);
		Account account = new Account();
		account.setRoles(new ArrayList<Role>());
		for (Map<String, Object> result : resultList) {
